/*
 ...
 Purpose:     Consumes a formatted DNA sequence file
              ...
              sequence file is a txt file
 Author:		Muhamad Izzul Syahmi 
 Date:			October 6, 2018
 */
...
void analyze_segments(char * sample_segment, char ** candidate_segments, int number_of_candidates)
{
	...
	/* Hint: Check to see if any candidate segment(s) are a perfect match, and report them
	  ...
	  /*Check which candidate segment match pefectly with the sample segment and printf the Candidate's name if it matches */
	for (i = 0; i < number_of_candidates; ++i) {
		if (strcmp(sample_segment, candidate_segments[i]) == 0) {
			...
			/*If none of the candidate segments matches perfectly, proceed with calculating the highest scores for each candidate segments, and print the result */
...
int calculate_score( char * sample_segment, char * candidate_segment)
{
	...
	char *candidate_amino_acid = (char*)malloc(13); /*To store the amino acid for candidate segments*/
	char *sample_amino_acid = (char*)malloc(13); /*To store the amino acid for sample segments*/
	int k;
	int i;
	int j;
	int h;

	for (k = 0; k < candidate_length; k += 3) {
		if (candidate_length - k >= sample_length-1) {

			temp_score = 0;
			for (i = 0; i < sample_length-1; i += 3) {

				/* assign amino acid for each codon for candidate segments*/
				for (h = 0; h < NUMBER_OF_CODONS;h++) {
					if (codon_codes[h][0] == candidate_segment[i + k] &&
						codon_codes[h][1] == candidate_segment[i + k + 1] &&
						codon_codes[h][2] == candidate_segment[i + k + 2]) {

						strcpy(candidate_amino_acid, codon_names[h]);
					}
				}

				/* assign amino acid for each codon for sample segments*/
				for (h = 0; h < NUMBER_OF_CODONS;h++) {
					if (codon_codes[h][0] == sample_segment[i] &&
						codon_codes[h][1] == sample_segment[i + 1] &&
						codon_codes[h][2] == sample_segment[i + 2]) {

						strcpy(sample_amino_acid, codon_names[h]);
					}
				}

				/*Assign score of 10 if the codon for candidate segment and sample segment matches*/
				if (candidate_segment[i + k] == sample_segment[i] &&
					candidate_segment[i + k + 1] == sample_segment[i + 1] &&
					candidate_segment[i + k + 2] == sample_segment[i + 2]) {
					temp_score+=10;					
				}
				
				/*Assign score of 5 if the codons have the same type of amino acid*/
				else if (strcmp(candidate_amino_acid,sample_amino_acid) == 0) {
					temp_score += 5;
				}
					...
					for (j = 0;j < 3; j++) {
						if (candidate_segment[i + j + k] == sample_segment[i + j]) {
							temp_score+=2; /*Add score of 2 if both nucleotides match*/

							
						}
						else if ((candidate_segment[i + j + k] == 'A' && sample_segment[i + j] == 'T') ||
							(candidate_segment[i + j + k] == 'T' && sample_segment[i + j] == 'A') || 
							(candidate_segment[i + j + k] == 'C' && sample_segment[i + j] == 'G') || 
							(candidate_segment[i + j + k] == 'G' && sample_segment[i + j] == 'C')) {

							temp_score++;  /*Add score of 1 if both nucleotides belong to a matching base pair*/							
						}
					}
				}
			}
		
			/* Check if the temporary score is higher than the previous score and replace the new score if true*/
			if (temp_score >= score) {
				score = temp_score;
			}
		}
